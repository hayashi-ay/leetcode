...
事前にtargetを超えるかチェックして、再帰呼び出しの回数をちょっと減らした。
```python
class Solution:
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        all_combinations = []
        ...
        def make_combination(candidate_index, current_sum):
            if current_sum == target:
                all_combinations.append(partial_combination.copy())
                return
            if candidate_index >= len(candidates):
                return
            make_combination(candidate_index + 1, current_sum)
            if current_sum + candidates[candidate_index] <= target:
                partial_combination.append(candidates[candidate_index])
                make_combination(candidate_index, current_sum + candidates[candidate_index])
                partial_combination.pop()
