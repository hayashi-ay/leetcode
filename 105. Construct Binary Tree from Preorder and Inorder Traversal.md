...
命名やinorder_indexをインクリメントするタイミングなど若干違うもののほとんど3rdと同じ。
...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        def build_tree(begin, end):
            ...
            preorder_index += 1
            root.left = build_tree(begin, inorder_index)
            root.right = build_tree(inorder_index + 1, end)
            return root
...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        num_left_nodes = inorder_index
        root.left = self.buildTree(preorder[1:num_left_nodes + 1], inorder[:inorder_index])
        root.right = self.buildTree(preorder[num_left_nodes + 1:], inorder[inorder_index + 1:])
        return root
```
