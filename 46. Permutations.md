...
class Solution:
    def permute(self, nums: List[int]) -> List[List[int]]:
        ...
        def make_permutations(candidates):
            if len(candidates) == 0:
                all_permutations.append(partial_permutation.copy())
                return
            for i in range(len(candidates)):
                partial_permutation.append(candidates[i])
                make_permutations(candidates[:i] + candidates[i + 1:])
                partial_permutation.pop()
        
        make_permutations(nums.copy())
        return all_permutations
```

> `if len(candidates) == 0`の部分はpartial_permutationの長さを見ても良いかも。

> `nums.copy()`のところは結局中の関数の処理が非破壊なのでなくても良いがあっても良い。
