...
class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        return max(max_amount_rob_first[-2], max_amount_not_rob_first[-1])
```
...
class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        max_amount_rob_first_two_before = nums[0]
        max_amount_rob_first_one_before = nums[0]
        max_amount_not_rob_first_two_before = 0
        max_amount_not_rob_first_one_before = nums[1]

        for i in range(2, len(nums)):
            # 先頭の家に強盗に入るケース
            current_max_rob_first = max(
                max_amount_rob_first_two_before + nums[i],
                max_amount_rob_first_one_before,
            )
            max_amount_rob_first_two_before = max_amount_rob_first_one_before
            max_amount_rob_first_one_before = current_max_rob_first

            # 先頭の家に強盗に入らないケース
            current_max_not_rob_first = max(
                max_amount_not_rob_first_two_before + nums[i],
                max_amount_not_rob_first_one_before,
            )
            max_amount_not_rob_first_two_before = max_amount_not_rob_first_one_before
            max_amount_not_rob_first_one_before = current_max_not_rob_first

        return max(max_amount_rob_first_two_before, max_amount_not_rob_first_one_before)
```
...
class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        def rob_house_in_line(nums):
            ...
            for i in range(2, len(nums)):
                current_max = max(nums[i] + max_amount_two_before, max_amount_one_before)
