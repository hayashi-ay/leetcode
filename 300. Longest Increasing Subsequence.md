...
Segment Tree
queryのときの範囲外の考慮とかが結構ふわっとしたままとりあえず動いたみたいな感じ。
座標圧縮は要素の重複も考慮しても良いが今回はしていない。

```python
class SegmentTree:
    def __init__(self, n, init_val=0):
        self.n = n
        self.items = [init_val] * (2 * n)
    
    # query max value in range [left, right)
    def query_max(self, left, right):
        res = 0
        left += self.n
        right += self.n
        while left < right:
            # 奇数の場合は親から見ると右のノードになり、親ノードは範囲外を含む
            if left % 2 == 1:
                res = max(res, self.items[left])
                left += 1
            # 奇数の場合は、範囲外なので左隣のノードをみる
            if right % 2 == 1:
                right -= 1
    ...
    def lengthOfLIS(self, nums: List[int]) -> int:
        def compress(nums):
            ...
            for index, num in enumerate(sorted_nums):
                num_to_sorted_index[num] = index

            compressed_nums = [0] * len(nums)
            for index, num in enumerate(nums):
                compressed_nums[index] = num_to_sorted_index[num]
            return compressed_nums

        compressed_nums = compress(nums)
        segtree = SegmentTree(len(compressed_nums), 0)
        for num in compressed_nums:
            max_lis_lower_than_num = segtree.query_max(0, num)
            segtree.update(num, max_lis_lower_than_num + 1)
        return segtree.query_max(0, len(nums))
```
