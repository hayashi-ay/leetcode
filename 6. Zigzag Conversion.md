...
次にどのrowに追加すべきかをstepで管理して、ループの中の処理をシンプルにした版。
あと文字列結合をなくしてjoinするように。
...
class Solution:
    def convert(self, s: str, num_rows: int) -> str:
        ...
        intermediates = [ [] for _ in range(num_rows) ]
        current_row = 0
        step = 1 # 下方向が1, 上方向が-1
        for c in s:
        ...
        return "".join([ c for row in intermediates for c in row ])
```
