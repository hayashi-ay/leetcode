...
class Solution:
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        if head.next is None:
            return head
        parent_after_reverse = head.next
        head.next = None
        root_after_reverse = self.reverseList(parent_after_reverse)
        parent_after_reverse.next = head
        return root_after_reverse
```

`next_node = parent_after_reverse = head.next`みたいな感じでも良いかも？
...
> 2つ目のループのcurrentは実際は１つ前のノードなのでprevの方が良い。
